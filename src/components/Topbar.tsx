...
export function Topbar({ activePage }: { activePage: PageId }) {
  return (
    <header className="sticky top-0 z-20 border-b border-white/70 bg-lavender/80 px-4 py-4 backdrop-blur-xl md:px-8">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden items-center gap-2 rounded-2xl border border-white bg-white px-3 py-2 shadow-sm md:flex">
            <Search className="h-4 w-4 text-slate-400" />
            <input className="w-56 bg-transparent text-sm outline-none" placeholder="Search records, tests, plans" />
          </div>
          <button className="rounded-2xl bg-white p-3 text-navy shadow-sm transition hover:bg-lavender">
