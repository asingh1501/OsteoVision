...
export function TestResultCard({ item, selected, onSelect }: { item: TestHistoryItem; selected?: boolean; onSelect?: () => void }) {
  return (
    <button onClick={onSelect} className={`card w-full p-4 text-left transition hover:-translate-y-0.5 ${selected ? 'ring-2 ring-violet' : ''}`}>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {item.changedMarkers.map((marker) => (
          <span key={marker} className="rounded-full bg-lavender px-2.5 py-1 text-xs font-semibold text-slate-600">{marker}</span>
        ))}
      </div>
    </button>
  );
