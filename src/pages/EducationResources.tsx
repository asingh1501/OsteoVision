      ...
      {selected && (
        <div className="card p-6">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
            <div>
              <span className="rounded-full bg-slate-50 px-3 py-1.5 text-sm font-bold text-violet">{selected.category}</span>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {articles.map((item) => (
          <div key={item.title} className="card p-5">
            ...
            <span className="mt-4 inline-flex rounded-full bg-slate-50 px-2.5 py-1 text-xs font-bold text-violet">{item.category}</span>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {videos.map((item) => (
            <button key={item.title} onClick={() => setSelectedTitle(item.title)} className="card flex items-center gap-4 p-5 text-left transition hover:-translate-y-0.5 hover:shadow-xl">
              <div className="rounded-xl bg-slate-100 p-4 text-violet"><PlayCircle className="h-8 w-8" /></div>
              <div>
