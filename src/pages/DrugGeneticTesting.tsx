    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <div className="card p-6">
          ...
          {scheduled && <p className="mt-3 rounded-2xl bg-emerald-50 p-3 text-sm font-bold text-emerald-700">{scheduled}</p>}
        </div>
      </div>
        ...
        {filtered.map((med) => (
          <div key={med.name} className="card p-5">
            ...
            <Button className="mt-5 w-full" variant="secondary" onClick={() => setDiscussionList((current) => current.includes(med.name) ? current : [...current, med.name])}>
              {discussionList.includes(med.name) ? 'Added to discussion' : 'Discuss with clinician'}
            </Button>
          </div>
        ))}
      </div>
