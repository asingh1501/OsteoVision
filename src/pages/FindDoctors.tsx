    ...
    <div className="space-y-6">
      <div className="card p-6">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          ...
          <select value={sort} onChange={(event) => setSort(event.target.value)} className="rounded-xl border border-slate-300 bg-white p-3 font-semibold text-navy">
      ...
      <div className="card p-6">
        ...
        <div className="mt-4 flex items-center gap-3 rounded-2xl border border-slate-300 bg-white px-4 py-3">
        ...
        {filtered.map((doctor) => (
          <div key={doctor.name} className="card p-5">
            ...
            <div className="mt-5 grid gap-2 md:grid-cols-2">
              <Button onClick={() => setContacted((prev) => prev.includes(doctor.name) ? prev : [...prev, doctor.name])}>{contacted.includes(doctor.name) ? 'Requested' : 'Contact'}</Button>
              <Button
                variant="secondary"
